...
public class StreamsIngest {
    ...
    @SneakyThrows
    private AvroSerde<City> citySerde(final Properties envProps) {
        Map<String, Object> ordersConfig = new HashMap<>();
        String schemaRegistryUrl = String.format("http://%s:8080", InetAddress.getLocalHost().getHostAddress()); // Issues with localhost
        ordersConfig.put(SerdeConfig.REGISTRY_URL, schemaRegistryUrl);
